...
class SgdgOrderingSettingsComponent extends wp.element.Component<SgdgOrderingSettingsComponentProps, SgdgOrderingSettingsComponentState> {
	public constructor( props: SgdgOrderingSettingsComponentProps ) {
		super( props );
		let valueBy = this.props.editor.getAttribute( this.props.name + '_by' ) as string;
		let valueOrder = this.props.editor.getAttribute( this.props.name + '_order' ) as string;
	...
	public render(): React.ReactNode {
		...
		const disabledBy = undefined === this.props.editor.getAttribute( this.props.name + '_by' );
		const disabledOrder = undefined === this.props.editor.getAttribute( this.props.name + '_order' );
		...
		return el( 'div', { className: 'sgdg-block-settings-row' }, [
			...
			el( 'select', { className: 'sgdg-block-settings-select', disabled: disabledOrder, onChange: ( e: React.FormEvent<Element> ) => {
				this.changeOrder( e );
			}, placeholder: sgdgBlockLocalize[ this.props.name ].default_order, type: 'number', value: valueOrder }, [
				el( 'option', { selected: 'ascending' === valueOrder, value: 'ascending' }, sgdgBlockLocalize.ordering_option_ascending ),
				el( 'option', { selected: 'descending' === valueOrder, value: 'descending' }, sgdgBlockLocalize.ordering_option_descending ),
			] ),
			el( 'label', { className: 'sgdg-block-settings-radio', for: this.props.name + '_by_time' }, [
				el( 'input', { checked: 'time' === valueBy, disabled: disabledBy, id: this.props.name + '_by_time', name: this.props.name + '_by', onChange: ( e ) => {
					this.changeBy( e );
			...
			el( 'label', { className: 'sgdg-block-settings-radio', for: this.props.name + '_by_name' }, [
				el( 'input', { checked: 'name' === valueBy, disabled: disabledBy, id: this.props.name + '_by_name', name: this.props.name + '_by', onChange: ( e ) => {
					this.changeBy( e );
