...
class Options_Proxy {
	...
	public function get( $name, $default_value = null ) {
		...
		if ( array_key_exists( $name . '_order', $this->overriden ) and array_key_exists( $name . '_by', $this->overriden ) ) {
			return ( $this->overriden[$name . '_by'] === 'name' ? 'name_natural' : 'modifiedTime' ) . ( $this->overriden[$name . '_order'] === 'ascending' ? '' : ' desc' );
		}
	...
	public function getOrder( $name, $default_value = null ) {
		if ( array_key_exists( $name . '_order', $this->overriden ) ) {
			return $this->overriden[ $name . '_order' ];
		}
		if ( array_key_exists( $name, $this->ordering_option_list ) ) {
			return $this->ordering_option_list[ $name ]->getOrder( $default_value );
		}
		return $default_value;
	}

	public function getBy( $name, $default_value = null ) {
		if ( array_key_exists( $name . '_by', $this->overriden ) ) {
			return $this->overriden[ $name . '_by' ];
		}
		if ( array_key_exists( $name, $this->ordering_option_list ) ) {
			return $this->ordering_option_list[ $name ]->getBy( $default_value );
		}
		return $default_value;
	}
}
